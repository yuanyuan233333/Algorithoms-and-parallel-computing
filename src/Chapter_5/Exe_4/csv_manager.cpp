...
{
   void csv_manager::parse_files(std::vector<Tax_Payer> & payers)
   {
   ...
   void csv_manager::parse_csv (const string & file_name)
   {
      ...
      {
         std::cerr << "Error accessing the file " << file_name << std::endl;
      }
   }

   void csv_manager::create_payers(std::vector<Tax_Payer> & payers)
   {
      for (const auto & row : fields)
      {
         int pos = find_payer(row[0], payers);
         if(pos == -1){
            payers.push_back(Tax_Payer(row[0],row[1],row[2]));
         }
         else{
            Tax_Payer & p = payers[static_cast<size_t>(pos)];
            p.update_address(row[2]);
         }
      }
   }

   void csv_manager::create_cards(std::vector<Tax_Payer> & payers)
   ...
   void csv_manager::create_transactions(std::vector<Tax_Payer> & payers)
      //Assumtion: the correspondig tax payers exist
   {
      for (const auto & row : fields)
      {
   ...
   void csv_manager::create_declarations(std::vector<Tax_Payer> & payers)
      //Assumtion: the correspondig tax payers exist
   {
      ...
      for (const auto & row : fields)
      {
         ...
         if(pos != -1){
            Tax_Payer & tp = payers[static_cast<size_t>(pos)];
            std::string::size_type sz;
            ...
            if(pos == -1){//if the declaration has not been created before
               ...
               Declaration & d = *all_declarations[static_cast<size_t>(pos)];
               tp.add_declaration(d);
            }
         }
      }
   ...
   int csv_manager::find_payer(const std::string & name, const std::vector<Tax_Payer> & payers)
   {
   ...
   int csv_manager::find_declaration(const std::string & declaration_number, const Tax_Payer::declarations_vector & declarations)
   {
